...
public final class DeliveryQueue {
    ...
    public synchronized void enqueue(DeliveryEntry entry) {
        if (contains(entry.id())) {
            throw new IllegalArgumentException("Delivery already queued: " + entry.id());
        }
        entries.add(entry);
    }

    public synchronized boolean contains(UUID deliveryId) {
    ...
    public synchronized Optional<DeliveryEntry> remove(UUID deliveryId) {
        for (int i = 0; i < entries.size(); i++) {
            if (entries.get(i).id().equals(deliveryId)) {
                return Optional.of(entries.remove(i));
            }
        }
        return Optional.empty();
    }

    public synchronized List<DeliveryEntry> snapshot() {
        return List.copyOf(entries);
    }

    public synchronized void replaceAll(Collection<DeliveryEntry> persistedEntries) {
        entries.clear();
