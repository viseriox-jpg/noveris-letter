...
public final class MailScreen extends Screen {
    ...
    @Override public boolean mouseClicked(double mx,double my,int button){if(super.mouseClicked(mx,my,button))return true;int l=left(),r=right(),ty=top()+44,tw=(r-l-36)/4;for(int i=0;i<4;i++){int x=l+12+i*(tw+4);if(in(x,ty,tw,28,mx,my)){selectTab(i);return true;}}
        ...
        if(tab<2){int y=top()+100;for(int i=0;i<entries.size();i++){if(in(l+24,y,r-l-48,34,mx,my))return true;y+=40;}if(in(l+24,bottom()-42,110,28,mx,my)&&page>0){page--;requestPage();return true;}if(in(r-134,bottom()-42,110,28,mx,my)&&page+1<totalPages){page++;requestPage();return true;}}
        return false;}
    ...
    private void drawLetters(GuiGraphics g,int l,int r,int b){g.drawString(font,tab==0?"CORRESPONDÊNCIAS RECEBIDAS":"CORRESPONDÊNCIAS ENVIADAS",l+24,top()+82,GOLD,false);int y=top()+105;for(MailSnapshotPayload.Entry e:entries){g.fill(l+24,y,r-24,y+34,FIELD);g.drawString(font,e.letterId().toString().substring(0,8),l+34,y+10,TEXT,false);y+=40;}g.drawString(font,"Página "+(page+1)+" / "+totalPages,l+24,b-30,MUTED,false);button(g,l+24,b-42,110,28,"ANTERIOR",false,0,0);button(g,r-134,b-42,110,28,"PRÓXIMA",false,0,0);}
