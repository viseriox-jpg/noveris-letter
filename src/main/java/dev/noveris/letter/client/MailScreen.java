...
public final class MailScreen extends Screen {
    ...
    private static final String[] COURIER_NAMES = {
            "MOSSBLOOM", "COATI", "PANDA-VERMELHO", "BOOPLET", "CAPIVARA"
    };
    private static final String[] COURIER_DESC = {
            "Natural e resistente", "Curioso e veloz", "Calmo e companheiro", "Pequeno e ágil", "Tranquilo e confiável"
    };
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (tab == 2) {
            ...
            for (int i = 0; i < onlinePlayers.size(); i++) {
                ...
                if (y + 22 <= playersBottom && mouseX >= mainRight + 14 && mouseX <= right - 14
        ...
        if (tab == 3) {
            ...
            int cardH = (cardBottom - cardTop - gap) / 2;
            for (int i = 0; i < COURIERS.length; i++) {
                int row = i / 3;
                int col = i % 3;
                int x = left + 24 + col * (cardW + gap);
                int y = cardTop + row * (cardH + gap);
                if (mouseInside(x, y, cardW, cardH, mouseX, mouseY)) {
    ...
    private void drawComposeButtons(GuiGraphics g, int left, int top, int bottom, int mouseX, int mouseY) {
        ...
        drawButton(g, anonymousX, y, 118, 27, anonymous ? "ANÔNIMA ✓" : "ANÔNIMA", anonymous, mouseX, mouseY);
    ...
    private void drawCourierSelection(GuiGraphics g, int left, int right, int top, int bottom, int mouseX, int mouseY) {
        ...
        g.drawString(font, "Escolha quem fará as próximas entregas pelo chão.", left + 24, top + 84, MUTED, false);
        int cardTop = top + 102;
        int cardBottom = bottom - 54;
        int gap = 10;
        int cardW = (panelWidth() - 48 - gap * 2) / 3;
        ...
        for (int i = 0; i < COURIERS.length; i++) {
            ...
            int x = left + 24 + col * (cardW + gap);
            int y = cardTop + row * (cardH + gap);
            boolean hover = mouseInside(x, y, cardW, cardH, mouseX, mouseY);
            ...
            g.drawCenteredString(font, COURIER_NAMES[i], x + cardW / 2, y + 18, selected ? GOLD : TEXT);
            g.drawCenteredString(font, COURIER_DESC[i], x + cardW / 2, y + 37, MUTED);
            g.drawCenteredString(font, "CAMINHA ATÉ VOCÊ", x + cardW / 2, y + 57, DIM);
            drawButton(g, x + 14, y + cardH - 31, cardW - 28, 25, selected ? "SELECIONADO" : "SELECIONAR", selected, mouseX, mouseY);
        }
    }
    ...
    private void drawPreview(GuiGraphics g, MailSnapshotPayload.Entry entry) {
        ...
        g.drawString(font, truncate(entry.sender() + " → " + entry.recipient(), 42), x + 12, y + 29, TEXT, false);
        g.drawString(font, truncate(entry.subject().isBlank() ? "Sem assunto" : entry.subject(), 42), x + 12, y + 45, MUTED, false);
        g.drawString(font, truncate(entry.preview(), 48), x + 12, y + 64, TEXT, false);
    ...
    private boolean mouseInside(int x, int y, int w, int h, double mouseX, double mouseY) {
