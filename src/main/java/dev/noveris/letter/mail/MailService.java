...
public final class MailService {
    ...
    public MailService(MinecraftServer server, CourierAppearanceRegistry appearances, int maxMessageLength) { this(server, appearances, maxMessageLength, 120); }
    public MailService(MinecraftServer server, CourierAppearanceRegistry appearances, int maxMessageLength, int maxSubjectLength) {
        this.server = Objects.requireNonNull(server, "server"); this.appearances = Objects.requireNonNull(appearances, "appearances");
        if (maxMessageLength < 1) throw new IllegalArgumentException("maxMessageLength must be positive");
        this.maxMessageLength = maxMessageLength; this.maxSubjectLength = maxSubjectLength;
    }
    private MailSavedData data() { return MailSavedData.get(server.overworld()); }
    public SendResult sendLetter(ServerPlayer sender, UUID recipientId, String content, long now) { return sendLetter(sender, recipientId, "", content, false, now); }
    public SendResult sendLetter(ServerPlayer sender, UUID recipientId, String subject, String content, long now) { return sendLetter(sender, recipientId, subject, content, false, now); }
    public SendResult sendLetter(ServerPlayer sender, UUID recipientId, String subject, String content, boolean anonymous, long now) {
        ...
        String recipientName = server.getPlayerList().getPlayer(recipientId) != null
        ...
        DeliveryEntry delivery = DeliveryFactory.create(id, sender.getUUID(), recipientId, DeliveryType.NORMAL, DeliveryPriority.NORMAL,
                appearances.resolveOrDefault(profile.selectedAppearance()).id(), now, now);
    ...
    public List<MailLetter> inbox(ServerPlayer viewer) { return lettersFor(viewer, data().inboxByPlayer().getOrDefault(viewer.getUUID(), List.of())); }
    public List<MailLetter> sent(ServerPlayer viewer) { return lettersFor(viewer, data().sentByPlayer().getOrDefault(viewer.getUUID(), List.of())); }
    private List<MailLetter> lettersFor(ServerPlayer viewer, List<UUID> ids) {
        return ids.stream().map(data().letters()::get).filter(Objects::nonNull).filter(letter -> letter.status() != MailStatus.DELETED)
                .filter(letter -> letter.senderId().equals(viewer.getUUID()) || letter.recipientId().equals(viewer.getUUID())).toList();
    }
    public boolean markDelivered(UUID letterId, long now) {
        ...
        if (letter.status() != MailStatus.DELIVERED && letter.status() != MailStatus.READ) { data.letters().put(letterId, letter.delivered(now)); data.markChanged(); }
        if (!inbox.contains(letterId)) { inbox.add(letterId); data.markChanged(); }
        return true;
    }
    ...
    public boolean archive(ServerPlayer owner, UUID letterId) {
        Optional<MailLetter> visible = findVisible(owner, letterId); if (visible.isEmpty()) return false;
    ...
    /** Permanently removes every letter involving the player from both sides and from the delivery queue. */
    public int clearHistory(UUID playerId) {
        MailSavedData data = data();
        List<UUID> ids = data.letters().values().stream()
                .filter(letter -> letter.senderId().equals(playerId) || letter.recipientId().equals(playerId))
                .map(MailLetter::id).toList();
        if (ids.isEmpty()) return 0;
        ids.forEach(id -> { data.letters().remove(id); data.deliveryQueue().snapshot().stream().filter(e -> e.letterId().equals(id)).map(DeliveryEntry::id).forEach(data.deliveryQueue()::remove); });
        data.inboxByPlayer().values().forEach(list -> list.removeIf(ids::contains));
        data.sentByPlayer().values().forEach(list -> list.removeIf(ids::contains));
        data.archivedByPlayer().values().forEach(list -> list.removeIf(ids::contains));
        data.markChanged(); return ids.size();
    }
