...
public final class MailNetwork {
    ...
    public static void open(ServerPlayer player, int tab) { int safeTab = Math.max(0, Math.min(3, tab)); PacketDistributor.sendToPlayer(player, new OpenMailScreenPayload(safeTab)); sendSnapshot(player, safeTab, 0); }
    private static void handleSnapshotRequest(RequestMailSnapshotPayload payload, IPayloadContext context) { context.enqueueWork(() -> { if (context.player() instanceof ServerPlayer player) sendSnapshot(player, payload.tab(), payload.page()); }); }
    private static void sendSnapshot(ServerPlayer player, int tab, int requestedPage) {
        ...
        var entries = letters.subList(from, Math.min(from + PAGE_SIZE, letters.size())).stream().map(letter -> new MailSnapshotPayload.Entry(letter.id(), letter.senderName(), letter.recipientName(), letter.subject(), preview(letter.content()), letter.status(), letter.sentAt())).toList();
        PacketDistributor.sendToPlayer(player, new MailSnapshotPayload(Math.max(0, Math.min(3, tab)), page, totalPages, entries));
    }
    private static String preview(String content) { String clean = content.replace('\n', ' ').replace('\r', ' ').trim(); return clean.length() <= 150 ? clean : clean.substring(0, 147) + "..."; }
    private static void handleSend(SendLetterPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            ...
            ServerPlayer online = sender.server.getPlayerList().getPlayers().stream().filter(p -> p.getGameProfile().getName().equalsIgnoreCase(recipientName)).findFirst().orElse(null);
            GameProfile profile = online != null ? online.getGameProfile() : sender.server.getProfileCache().get(recipientName).orElse(null);
            MailService.SendResult result = profile == null ? MailService.SendResult.INVALID_RECIPIENT : new MailService(sender.server, new CourierAppearanceRegistry(), 1500, 120).sendLetter(sender, profile.getId(), payload.subject(), payload.content(), payload.anonymous(), System.currentTimeMillis());
