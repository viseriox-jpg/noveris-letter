...
public final class MailNetwork {
    ...
    private static void handleSnapshotRequest(RequestMailSnapshotPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> { if (context.player() instanceof ServerPlayer player) sendSnapshot(player, payload.tab(), payload.page()); });
    }
    private static void sendSnapshot(ServerPlayer player, int tab, int requestedPage) {
        ...
        int totalPages = Math.max(1, (letters.size() + PAGE_SIZE - 1) / PAGE_SIZE);
        int page = Math.max(0, Math.min(requestedPage, totalPages - 1));
        int from = page * PAGE_SIZE;
        var entries = letters.subList(from, Math.min(from + PAGE_SIZE, letters.size())).stream()
                .map(letter -> new MailSnapshotPayload.Entry(letter.id(), letter.senderName(), letter.recipientName(), letter.subject(), preview(letter.content()), letter.status(), letter.sentAt())).toList();
